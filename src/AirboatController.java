...
public enum AirboatController {
	...
	POINT_AND_SHOOT(new VehicleController() {
		...
		public Logger logger = Logger.getLogger(AirboatController.class.getName());
		...
		public void update(VehicleServer server, double dt) {
			...
			logger.log(Level.parse("POINT AND SHOOT: "),String.format("%s vs. %s --> distanceSq = %.2f", pose.toString(), waypoint.toString(), distanceSq));


			if (distanceSq <= (3*3))
			{
				//Log.i("POINT_AND_SHOOT:", "Arrived Waypoint");
				logger.log(Level.parse("POINT AND SHOOT:"),"Arrived Waypoint");
